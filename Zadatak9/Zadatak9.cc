...
bool pomicanje(int i, int x, int y) {
    ...
    if (ploca[x][y].tip == -1) {
        // Ažuriraj polje na ploči
        ploca[figura[i].x][figura[i].y].tip = -1;
        figura[i].brojPoteza++;
        // Postavi novu poziciju figure
        figura[i].x = x;
        figura[i].y = y;
        // Postavi figuru na novo polje
        ploca[x][y].id = figura[i].id;
        ploca[x][y].tip = figura[i].tip;
        return true;
    } else {
        // Provjeri je li moguće jesti figuru ili je došlo do sukoba figura
        if (figura[i].tip > ploca[x][y].tip) {
            ...
            cout << "Figurice tipa " << figura[i].tip << " su jednake, jedna ispadne" << endl;
            figura[ploca[x][y].id].ziv = false;
        }
        // Ažuriraj broj poteza figure
        figura[i].brojPoteza++;
        // Postavi novu poziciju figure
        figura[i].x = x;
        figura[i].y = y;
